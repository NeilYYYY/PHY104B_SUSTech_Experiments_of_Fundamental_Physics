...
11. 作图什么都可以，不局限于origin，用python、matlab甚至excel都是可以的，只要能画出来符合要求的图；
12. 用AI写代码的前提是至少能看懂它写了点啥，不然跑出来的图用的可能甚至不是自己的数据；
13. 大实验的Pre注意不要超时，这学期是6min的展示和2min的问答，部分老师会在到时间后直接打断（我们组超时了不少，感谢老师不杀之恩（还有记得要脱稿；
14. to be Filled...
...
- 大实验拟合用的matlab，gpt写的代码，懒得一个字都没改（怎么那时没想到用python呢；
